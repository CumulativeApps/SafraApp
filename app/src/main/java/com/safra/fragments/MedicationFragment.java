...
import static com.safra.utilities.Common.HEALTH_RECORD_MEDICATION_DELETE;
import static com.safra.utilities.Common.HEALTH_RECORD_MEDICATION_LIST;
import static com.safra.utilities.Common.HEALTH_RECORD_MEDICATION_UPDATE_STATUS;
import static com.safra.utilities.Common.PAGE_START;
import static com.safra.utilities.Common.REQUEST_DELETE_HEALTH_MEDICATION_DELETE;
...
import com.safra.AddMedication;
import com.safra.R;
import com.safra.Safra;
import com.safra.adapters.MedicationRecyclerAdapter;
import com.safra.adapters.MedicationRecyclerAdapter;
import com.safra.databinding.FragmentAllergiesListBinding;
import com.safra.databinding.FragmentMedicationBinding;
...
public class MedicationFragment extends DialogFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding.rvMedicationList.setLayoutManager(new LinearLayoutManager(mActivity, RecyclerView.VERTICAL, false));
        binding.rvMedicationList.addItemDecoration(new SpaceItemDecoration(mActivity, RecyclerView.VERTICAL,
                1, R.dimen.recycler_vertical_offset, R.dimen.recycler_horizontal_offset, true));
        adapter = new MedicationRecyclerAdapter(mActivity, new MedicationRecyclerAdapter.OnItemClickListener() {
            @Override
            public void onDelete(MedicationListModel.Data.Medication item, int position) {
                DeleteDialog dialogD = new DeleteDialog();
                Bundle bundle = new Bundle();
                bundle.putString("request_key", REQUEST_DELETE_HEALTH_MEDICATION_DELETE);
            ...
            @Override
            public void onEdit(MedicationListModel.Data.Medication item, int position) {
                Intent i = new Intent(mActivity, AddMedication.class);
                ...
                i.putExtra("medName", item.getMedicine().getName());
                i.putExtra("quantity", item.getQuantity());
                i.putExtra("instruction", item.getInstructions());
            ...
            @Override
            public void onView(MedicationListModel.Data.Medication item, int position) {
            ...
            @Override
            public void changeStatus(View view, MedicationListModel.Data.Medication item, int position) {
                int finalStatus = -1;

                int currentStatus = item.status;
                ...
                changeStatusAPI(item.id, position,finalStatus);
//                setPopUpWindowForChangeStatus(view, item.getUserId(), item.getUserOnlineId(), item.getUserStatus());
            }

            private void changeStatusAPI(int id, int position,int finalStatus) {
...
//        dialogL.show(getChildFragmentManager(), LoadingDialog.TAG);
                ...
                AndroidNetworking
                        .post(BASE_URL + HEALTH_RECORD_MEDICATION_UPDATE_STATUS)
                        .addBodyParameter("user_token", isRemembered ? userSessionManager.getUserToken() : Safra.userToken)
                        .addBodyParameter("medication_id", String.valueOf(id))
                        .addBodyParameter("status", String.valueOf(finalStatus))
...
//                        dialogL.dismiss();
        ...
        binding.rvMedicationList.setAdapter(adapter);
...
//                getUsersFromDB();
        ...
        binding.rvMedicationList.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
...
//        });
        ...
        getChildFragmentManager().setFragmentResultListener(REQUEST_DELETE_HEALTH_MEDICATION_DELETE, this,
                (requestKey, result) -> {
...
//                        deleteUserOffline(userId,onlineId, position);
                        deleteMedication(userId, position);
...
//        binding.etSearch.setHint(LanguageExtension.setText("search_the_user", getString(R.string.search_the_user)));
        ...
        binding.tvEmptyState.setText(LanguageExtension.setText("no_medication_found", getString(R.string.no_medication_found)));
    }

    private void getUsersFromDB() {
        userList.clear();

//        userList.addAll(dbHandler.getMedication(isRemembered ? userSessionManager.getUserId() : Safra.userId));
//
//        for (MedicationListModel.Data.Medication userItem : userList) {
...
//    }
    ...
    private void getMedication(int pPosition) {
        binding.srlManageProject.setRefreshing(currentPage == PAGE_START);

        isNextPageCalled = true;
        AndroidNetworking
                .post(BASE_URL + HEALTH_RECORD_MEDICATION_LIST)
                .addBodyParameter("user_token", isRemembered ? userSessionManager.getUserToken() : Safra.userToken)
...
//                        Log.e(TAG, "onResponse: " + response);
                        try {
                            ...
                            if (success == 1) {
                                ...
                                JSONArray users = data.getJSONArray("medication");
...
//                                    pPosition = -1;
                                ...
                                if (users.length() > 0) {
                                    List<MedicationListModel.Data.Medication> uList = new ArrayList<>();
                                    for (int i = 0; i < users.length(); i++) {
                                        JSONObject user = users.getJSONObject(i);
                                        JSONObject medicine = user.getJSONObject("medicine");
                                        String medicineName = medicine.getString("name");
//                                        MedicationListModel.Data.Medication userItem = new MedicationListModel.Data.Medication();
                                        MedicationListModel.Data.Medication userItem = new Gson().fromJson(user.toString(), MedicationListModel.Data.Medication.class);


                                        userItem.getMedicine().getName();
                                        System.out.println("getName():-" +userItem.getMedicine().getName());
...
//        userList.clear();
//        userList.add(new MedicationListModel.Data.Medication(1, "John Doe", "02/10/2021, 11:52 AM", "John.doe@safra.cloud", "Moderator", 10, 10));
//        userList.add(new MedicationListModel.Data.Medication(2, "Jane Doe", "02/10/2021, 11:52 AM", "John.doe@safra.cloud", "Moderator", 10, 10));
//
//        adapter.notifyDataSetChanged();
//        checkForEmptyState();
    }
    ...
    public void deleteMedication(long userId, int position) {
        LoadingDialogExtension.showLoading(mActivity, LanguageExtension.setText("deleting_progress", getString(R.string.deleting_progress)));
//        LoadingDialog dialogL = new LoadingDialog();
//        dialogL.setCancelable(false);
//        Bundle bundle = new Bundle();
...
//        dialogL.show(getChildFragmentManager(), LoadingDialog.TAG);
        ...
        AndroidNetworking
                .post(BASE_URL + HEALTH_RECORD_MEDICATION_DELETE)
                .addBodyParameter("user_token", isRemembered ? userSessionManager.getUserToken() : Safra.userToken)
                .addBodyParameter("medication_id", String.valueOf(userId))
//                .setTag("delete-user-api")
                .build()
                .getAsJSONObject(new JSONObjectRequestListener() {
                    @Override
