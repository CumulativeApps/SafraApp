...
public class AllergiesRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public int getItemViewType(int position) {
        int VIEW_PROGRESS = 0;
        return userList.get(position) != null ? VIEW_ALLERGIES : VIEW_PROGRESS;
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_ALLERGIES) {
            RecyclerAllergiesBinding binding = RecyclerAllergiesBinding.inflate(inflater, parent, false);
            return new AllergiesRecyclerAdapter.AllergiesViewHolder(binding);
        } else {
            ItemLoadingBinding binding = ItemLoadingBinding.inflate(inflater, parent, false);
            return new AllergiesRecyclerAdapter.ProgressViewHolder(binding);
        }    }

    @Override
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof AllergiesRecyclerAdapter.AllergiesViewHolder)
            ((AllergiesRecyclerAdapter.AllergiesViewHolder) holder).bindView(userList.get(position));
    }
    ...
    class AllergiesViewHolder extends RecyclerView.ViewHolder {
        RecyclerAllergiesBinding binding;

        public AllergiesViewHolder(@NonNull RecyclerAllergiesBinding binding) {
